 ...
 * 
 * <hr>
 * This is a highly-modified Android class that now works with jdesktop timing classes.
 * 
...
public final class AnimatorSet extends Animator {
    ...
    private Animator mDelayAnim = null;
    ...
    public void addTarget(TimingTarget target) {
        for (Node node : mNodes) {
            ...
            if (animation instanceof AnimatorSet) {
                ((AnimatorSet)animation).addTarget(target);
            }
        }
    }
    ...
    public boolean cancel() {
        mTerminated = true;
        if (isStarted()) {
            CopyOnWriteArrayList<TimingTarget> tmpListeners = null;
            if (f_targets != null) {
                tmpListeners = (CopyOnWriteArrayList<TimingTarget>) f_targets.clone();
                for (TimingTarget listener : tmpListeners) {
        ...
        if (isStarted()) {
            if (mSortedNodes.size() != mNodes.size()) {
                ...
                for (Node node : mSortedNodes) {
                    ...
                    node.animation.addTarget(mSetListener);
                }
            }
            if (mDelayAnim != null) {
                mDelayAnim.cancel();
    ...
    @SuppressWarnings("unchecked")
	@Override
    public AnimatorSet setDuration(long duration, TimeUnit unit) {
        if (duration < 0) {
        ...
        for (Node node : mNodes) {
            ...
            node.animation.setDuration(duration, unit);
        }
        mDuration = duration;
        return this;
    }
    ...
    public AnimatorSet setDuration(long duration)
    {
        ...
        for (Node node : mNodes) {
            // TODO: don't set the duration of the timing-only nodes created by AnimatorSet to
            // insert "play-after" delays
            node.animation.setDuration(duration, TimeUnit.MILLISECONDS);
        }
        mDuration = duration;
        return this;
    }
    ...
    public void start() {
        ...
        for (int i = 0; i < numSortedNodes; ++i) {
            ...
            CopyOnWriteArrayList<TimingTarget> oldListeners = node.animation.getTargets();
            if (oldListeners != null && oldListeners.size() > 0) {
                final CopyOnWriteArrayList<TimingTarget> clonedListeners = new
                		CopyOnWriteArrayList<TimingTarget>(oldListeners);

                for (TimingTarget listener : clonedListeners) {
                    if (listener instanceof DependencyListener ||
                            listener instanceof AnimatorSetListener) {
                        node.animation.removeTarget(listener);
                    }
                }
            }
        }
        ...
        for (int i = 0; i < numSortedNodes; ++i) {
            ...
            if (node.dependencies == null || node.dependencies.size() == 0) {
                ...
                for (int j = 0; j < numDependencies; ++j) {
                    ...
                    dependency.node.animation.addTarget(
        ...
        if (mStartDelay <= 0) {
            ...
            mDelayAnim = new Animator();
            mDelayAnim.setDuration(mStartDelay, TimeUnit.MILLISECONDS);
            mDelayAnim.addTarget(new TimingTargetAdapter() {
                boolean canceled = false;
                @Override
                public void cancel(Animator anim) {
                    canceled = true;
                }
                @Override
                public void end(Animator anim) {
                    if (!canceled) {
    ...
    public AnimatorSet clone() {
        ...
        for (Node node : mNodes) {
            ...
            CopyOnWriteArrayList<TimingTarget> cloneListeners = nodeClone.animation.getTargets();
            if (cloneListeners != null) {
            	CopyOnWriteArrayList<TimingTarget> listenersToRemove = null;
                for (TimingTarget listener : cloneListeners) {
                    if (listener instanceof AnimatorSetListener) {
                        if (listenersToRemove == null) {
                            listenersToRemove = new CopyOnWriteArrayList<TimingTarget>();
                        }
                        listenersToRemove.add(listener);
                    }
                }
                if (listenersToRemove != null) {
                    for (TimingTarget listener : listenersToRemove) {
     ...
     */
    private static class DependencyListener implements TimingTarget {
        ...
        private void startIfReady(Animator dependencyAnimation) {
            ...
            for (int i = 0; i < numDependencies; ++i) {
                ...
                if (dependency.rule == mRule &&
                    ...
                    dependencyAnimation.removeTarget(this);
                    break;
                }
            }
    ...
    private class AnimatorSetListener implements TimingTarget {
		...
		public void cancel(Animator animation) {
			if (!mTerminated) {
                ...
                if (mPlayingSet.size() == 0) {
                    if (f_targets != null) {
                        int numListeners = f_targets.size();
                        for (int i = 0; i < numListeners; ++i) {
                        	f_targets.get(i).cancel(mAnimatorSet);
                        }
                    }
                }
            }
            ...
            if (!mTerminated) {
                ...
                if (allDone) {
                    ...
                    if (f_targets != null) {
                        ...
                        int nuf_targets = tmpListeners.size();
                        for (int i = 0; i < nuf_targets; ++i) {
                            tmpListeners.get(i).end(mAnimatorSet);
                        }
                    }
    ...
    public class Builder {
        ...
        public Builder after(long delay) {
            ...
            Animator anim = new Animator().setDuration(delay, TimeUnit.MILLISECONDS);
            after(anim);
            return this;
        }
