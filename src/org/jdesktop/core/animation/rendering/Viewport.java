...
public final class Viewport {
   ...
   */
  @Modified(author="Phil Brown", summary="removed reference to Animator.Builder")
  public void animateHorizontalMovement(Go toward) {
    ...
    if (pixelsToMove > 0) {
    	...
    	anim.setDuration(f_rate.getMovementRateNanosPerPixel() * pixelsToMove, TimeUnit.NANOSECONDS);
    	anim.setInterpolator(f_moveI);
    	anim.addTarget(new Mover(pixelsToMove, true, f_x, toward == Go.RIGHT));
      f_moveHorizontalAnimator = anim;
  ...
  public void animateVerticalMovement(Go toward) {
    ...
    if (pixelsToMove > 0) {
      f_moveVerticalAnimator = new Animator();
      f_moveVerticalAnimator.setDuration(f_rate.getMovementRateNanosPerPixel() * pixelsToMove, TimeUnit.NANOSECONDS);
      f_moveVerticalAnimator.setInterpolator(f_moveI);
      f_moveVerticalAnimator.addTarget(new Mover(pixelsToMove, false, f_y, toward == Go.DOWN));
